...
```{r}
dados <- read.table('https://pastebin.com/raw/RshQ8KjB', header = T)
head(dados, n = 2)
summary(dados)
```
...
head(data.frame(ajuste$y, dadosajuste$Status),n = 5)
```
...
```{r}
head(round(pval2 * 1000), n = 3) # Escores de crédito
```
...
head(classp0.5, n = 3) # Classificações usando o ponto de corte 0,5.
head(data.frame(pval2, classp0.5), n = 3) # Probabilidades estimadas e classificações.
```
